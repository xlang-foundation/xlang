...
namespace X
{
	...
	bool RemotingProxy::Connect()
	{
		...
		bool bOK = mWBuffer->ClientConnect(mHostUseGlobal,m_port,shmKey,SM_BUF_SIZE, timeoutMS);
		if (bOK)
		{
			bOK = mRBuffer->ClientConnect(mHostUseGlobal,m_port, shmKey+1,SM_BUF_SIZE, timeoutMS, false);
			if (bOK)
			{
				ShapeHandsToServer();
			}
